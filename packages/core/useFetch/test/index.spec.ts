...
describe.skipIf(isBelowNode18)("useFetch", () => {
  ...
  it("should throw error", async () => {
    ...
    const error1 = await useFetch("https://example.com?status=400", options)
      .execute(true)
      ?.catch((err) => err);
    const error2 = await useFetch("https://example.com?status=600", options)
      .execute(true)
      ?.catch((err) => err);

    expect(error1).toBe("Bad Request");
  ...
  it("async chained beforeFetch and afterFetch should be executed in order", async () => {
    ...
    const { data } = await useMyFetch("test?json", {
      method: "GET",
      async beforeFetch({ options }) {
